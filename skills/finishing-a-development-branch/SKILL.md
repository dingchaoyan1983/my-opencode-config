...
If tests fail on the merged result: stop and investigate — nothing has
been pushed, so the merge is local and recoverable.
...
Keep the branch in place — your human partner iterates on PR feedback
there.
...
| "The PR is up, so the branch is clutter now" | PR feedback gets fixed in that branch. It stays until the work lands. |
